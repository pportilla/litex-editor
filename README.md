...
Packaging files live under `packaging/arch`. Desktop and AppStream metadata live
under `resources/` so the upstream CMake install is independent of the Arch
packaging directory. Before submission, the upstream URL, signed release tag, and
release checksum must be real. See `PACKAGING.md` for the release and clean
chroot checklist.
